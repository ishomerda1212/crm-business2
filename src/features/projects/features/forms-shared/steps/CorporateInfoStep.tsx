...
export const CorporateInfoStep = () => {
  ...
  if (!hearingContext && !simpleContext) {
    throw new Error('CorporateInfoStep must be used within a form context');
  }

  const context = (hearingContext || simpleContext)!;
  const { data, updateCorporateInfo } = context;

  const handleSelect = (event: ChangeEvent<HTMLSelectElement>) => {
    updateCorporateInfo({ corporationType: event.target.value as typeof corporationTypes[number] });
  };
